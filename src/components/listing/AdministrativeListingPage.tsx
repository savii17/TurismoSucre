...
type Props = { title: string; description: string; actionLabel: string; items: AdministrativeListing[]; showDate?: boolean; detailPath?: string };

export default function AdministrativeListingPage({ title, description, actionLabel, items, showDate, detailPath }: Props) {
  ...
  return <section className="admin-listing" aria-labelledby="listing-title">
    ...
    <div className="admin-cards">{visibleItems.map((item) => <article className="admin-card" key={item.id}><div className="admin-card__media">{item.image ? <img src={item.image} alt="" /> : <span aria-hidden="true">⌖</span>}<b className={`status-badge status-badge--${item.status.toLowerCase()}`}>{item.status === "ACTIVO" ? "Activo" : "Inactivo"}</b></div><div className="admin-card__body"><p className="admin-card__category">{item.category}</p><h2>{item.name}</h2>{showDate && <p className="admin-card__date">◷ {item.date}</p>}<p className="admin-card__location">⌖ {item.location}</p><p className="admin-card__description">{item.description}</p>{item.services && <div className="admin-card__services">{item.services.slice(0, 3).map((service) => <span key={service}>{service}</span>)}</div>}<div className="admin-card__meta">{item.rating != null && <span>★ {item.rating.toFixed(1)} {item.reviews != null && <small>({item.reviews})</small>}</span>}<strong>{item.price}</strong></div>{detailPath ? <Link className="admin-card__details" to={`${detailPath}/${encodeURIComponent(item.id)}`}>Ver detalles <span>→</span></Link> : <button className="admin-card__details" type="button">Ver detalles <span>→</span></button>}</div></article>)}</div>
